# Multi-stage build for CacheForge
# Stage 1: Build stage
...
RUN addgroup -g 1000 cacheforge && \
    adduser -D -u 1000 -G cacheforge cacheforge && \
    mkdir -p /data /app/logs /config && \
    chown -R cacheforge:cacheforge /data /app /config && \
    chmod 755 /data /app /config /app/logs

# Copy the binary
COPY --from=builder /app/cacheforge /cacheforge
RUN chmod +x /cacheforge
...
# Default command
ENTRYPOINT ["/cacheforge"]
CMD ["--config", "/config/cacheforge.yaml", "--protocol", "resp"]
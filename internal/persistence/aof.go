...
func (aof *AOFManager) Open() error {
	...
	logPath := filepath.Join(aof.dataDir, "cacheforge.aof")
...
func (aof *AOFManager) Compact(ctx context.Context, currentData map[string]interface{}) error {
	...
	tempPath := filepath.Join(aof.dataDir, "cacheforge.aof.tmp")
	...
	originalPath := filepath.Join(aof.dataDir, "cacheforge.aof")
